...
export const InventoryManager: React.FC<InventoryManagerProps> = ({ onSelectTab }) => {
  ...
  const { inventory, weighings, addInventoryItem, updateInventoryItem } = useData();
  ...
  const companyInventory = inventory.filter(i => i.companyId === (activeCompany?.id || 'comp_galpon_real'));
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {companyInventory.map((item) => {
          ...
          return (
            <div 
              key={item.id}
              className="bg-slate-900 border border-slate-800 hover:border-slate-700 rounded-2xl p-5 shadow-lg space-y-4 relative"
            >
              <div className="flex items-start justify-between">
                <div>
                  <span className="text-[10px] uppercase font-extrabold text-emerald-400 bg-emerald-950 px-2.5 py-1 rounded-full border border-emerald-800/80">
                    {item.name}
                  </span>
              ...
              <div className="bg-slate-950 p-3.5 rounded-xl border border-slate-800 space-y-2">
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-400">Total Pollos:</span>
                  <span className="font-black text-emerald-400 font-mono text-base">{item.headCount} aves</span>
                </div>

                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-400">Peso Estimado Total:</span>
                  <span className="font-bold text-slate-200 font-mono">{item.totalWeight.toFixed(1)} kg</span>
                </div>

                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-400">Peso Promed. Ave:</span>
                  <span className="font-bold text-slate-400 font-mono">{item.averageWeight.toFixed(2)} kg</span>
                </div>
              </div>

              {isLowStock && (
                <div className="flex items-center space-x-1.5 text-xs text-amber-400 bg-amber-950/60 p-2 rounded-xl border border-amber-800/80 font-semibold">
      ...
      {(() => {
        ...
        return (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-xl overflow-hidden space-y-3 p-5">
                  ...
                  {kardexRows.map((row) => (
                    <tr key={row.id} className="hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="p-3 font-bold">
                        {row.type === 'ENTRADA' ? (
                          <span className="bg-emerald-950 text-emerald-400 border border-emerald-800 px-2.5 py-0.5 rounded-full text-[10px] inline-flex items-center gap-1 font-bold">
                            <ArrowDownLeft className="w-3 h-3 text-emerald-400" />
                            <span>ENTRADA</span>
                          </span>
                        ) : (
                          <span className="bg-amber-950 text-amber-400 border border-amber-800 px-2.5 py-0.5 rounded-full text-[10px] inline-flex items-center gap-1 font-bold">
                            <ArrowUpRight className="w-3 h-3 text-amber-400" />
                            <span>SALIDA (VENTA)</span>
                          </span>
                        )}
                      </td>
                      <td className="p-3 font-bold text-white">{row.galpon}</td>
                      ...
                      <td className="p-3 text-slate-300 max-w-xs truncate">{row.reference}</td>
                      <td className="p-3 text-slate-400">{row.operator}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <form onSubmit={handleSave} className="space-y-4 text-xs">
              <div>
                <div className="flex justify-between items-center mb-1.5">
                  <label className="block text-slate-300 font-semibold">Seleccionar Galpón de Destino *</label>
                  <button
                    type="button"
                ...
                {showAddGalpon && (
                  <div className="bg-slate-950 p-2.5 rounded-xl border border-emerald-500/40 mb-3 space-y-2 animate-fade-in">
                    <label className="text-[10px] text-slate-400 font-bold block">Nombre del nuevo Galpón:</label>
                    <div className="flex items-center space-x-2">
                      <input
                        type="text"
                        ...
                        className="flex-1 bg-slate-900 border border-slate-700 text-white rounded-lg px-2.5 py-1.5 text-xs outline-none focus:border-emerald-500 font-bold"
                      />
                      <button
                        type="button"
                ...
                <select
                  ...
                  className="w-full bg-slate-950 border border-slate-700 text-white rounded-xl px-3 py-2.5 text-xs font-bold outline-none focus:border-emerald-500 cursor-pointer"
                >
              ...
              <div>
                <label className="block text-slate-300 mb-1 font-semibold">Cantidad de Pollos a Ingresar *</label>
                <input
                  type="number"
                  required
                  min="1"
                  ...
                  placeholder="ej. 500"
                  className="w-full bg-slate-950 border border-slate-700 text-emerald-400 font-extrabold text-lg rounded-xl px-3 py-2.5 outline-none focus:border-emerald-500 font-mono"
                />
              </div>

              <div>
                <label className="block text-slate-300 mb-1 font-semibold">Peso Promedio Estimado por Pollo (kg)</label>
                <input
                  type="number"
                  step="0.05"
                  value={estimatedAvgWeight}
                  onChange={(e) => setEstimatedAvgWeight(parseFloat(e.target.value) || 2.4)}
                  className="w-full bg-slate-950 border border-slate-700 text-white font-bold rounded-xl px-3 py-2 outline-none focus:border-emerald-500 font-mono"
                />
              </div>
